"""Background job abstraction for long-running domain steps.

Heavy steps (bio: alignment, mutation calling; gwas: COJO, SuSiE, SMR) can
run for seconds to hours. The agent loop never blocks on them: it starts a
job, gets a job_id back immediately, and polls until done. Mock mode vs
real mode both honour the same contract so swapping them never changes how
...
_executor = ThreadPoolExecutor(max_workers=8, thread_name_prefix="agentcore-job")
_jobs: dict[str, "Job"] = {}
...
def start_job(
    ...
    """Submit fn(**kwargs) to the background thread pool. Returns a job_id immediately."""
