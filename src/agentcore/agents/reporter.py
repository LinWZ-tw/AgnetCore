...
One Reporter serves both domains: the agent loop, tool list, and
Markdown->HTML rendering are ~100% shared. Only the final content-generation
step branches on `checkpoint["domain"]` (set by the Planner as soon as it
classified the input, see agentcore/agents/planner.py):
  - "bio": calls domains.bio.figures.generate_figures_for_run() for a PNG
    figure gallery, and domains.bio.export.export_workflow() for
    reproduce.sh/Snakefile/methods.md.
  - "gwas": links to the network.png/network.html already produced by the
    gwas domain's `visualize` step (a normal dispatched step run by the MR
    stage agent, not something the Reporter generates itself).
"""
...
def _figures_html(figure_entries: dict[str, list[dict[str, str]]], report_dir: Path) -> str:
    """Bio-only: PNG/interactive figure gallery, grouped by sample."""
...
def _network_html(report_dir: Path) -> str:
    """GWAS-only: link to the causal-network PNG/HTML already produced by the
    `visualize` step (dispatched by the MR stage agent, not by the Reporter)."""
    png = report_dir / "network.png"
    html = report_dir / "network.html"
    if not png.exists() and not html.exists():
        return ""
    parts = ['<section id="figures">', "<h2>Causal Network</h2>", '<div class="figure-grid">']
    if html.exists():
        parts.append(
            f'<figure class="interactive"><iframe src="network.html" '
            f'style="width:100%;height:560px;border:none;"></iframe>'
            f"<figcaption>Interactive causal network</figcaption></figure>"
        )
    if png.exists():
        parts.append(
            '<figure><img src="network.png" alt="Causal network">'
            "<figcaption>Causal network (static)</figcaption></figure>"
        )
...
<p style="font-size:12px;color:var(--muted);">
  Generated by the merged translational-agent framework &mdash; run <code>{run_id}</code>{footer_extra}
</p>
</main>
</body>
</html>"""
...
def _make_toc(md: str, has_extra_section: bool) -> str:
    links = []
...
def _build_html(
    ...
    toc = _make_toc(md, has_extra_section=bool(extra_section))
...
def run(
    *,
    ...
    provider.send_user_text(
        ...
        f"Call read_checkpoint to retrieve all step/stage results, then write the full Markdown report."
    )
    ...
    domain = checkpoint.get("domain") or "bio"
    ...
    if domain == "bio":
        ...
        try:
            from ..domains.bio import figures as fig_module
        ...
        extra_section = _figures_html(figure_paths, rdir)

        # Generate reproducibility artifacts (shell script, Snakefile, methods.md)
        try:
            from ..domains.bio import export as _export
        ...
        extra_section = _network_html(rdir)

    md_path = rdir / "report.md"
    html_path = rdir / "report.html"
    md_path.write_text(report_text, encoding="utf-8")
    html_path.write_text(_build_html(report_text, run_id, extra_section, footer_extra), encoding="utf-8")
