"""Bio-domain tool schemas and dispatch table (WES + scRNA).
...
or from the gwas domain. `agentcore/tools.py` imports the pieces it needs
...
INSPECT_DATA_SOURCE: dict[str, Any] = {
    ...
    "description": (
        ...
        "Call this when the input looks like sequencing data (a directory, archive, or .h5/.h5ad file) "
        "rather than a single tabular summary-statistics file. "
...
DISPATCH_WORKER_TOOL: dict[str, Any] = {
    "name": "dispatch_worker",
    "description": (
        "Dispatch a bioinformatics pipeline branch to the appropriate worker agent (Layer 2). "
...
_LOCATE_FASTQ_PAIRS: dict[str, Any] = {
...
WORKER_TOOLS: list[dict[str, Any]] = [
    _START_JOB, _CHECK_JOB, _GET_RESULT, _REQUEST_CONFIRMATION, _READ_CHECKPOINT, _LOCATE_FASTQ_PAIRS,
]
...
def _job_on_complete(run_id: str, step_name: str, phase: str | None, exec_mode: str, args: dict[str, Any]):
...
def dispatch(run_id: str, name: str, tool_input: dict[str, Any], auto_approve: bool, phase: str | None = None) -> dict[str, Any]:
    """Execute a single bio-domain tool call. Returns the dict to JSON-encode as tool_result content.

    `phase` identifies which worker branch ("wes" or "scrna") is making this call, passed
    through by wes_agent.py/scrna_agent.py so it lands in the state.json step record.
    """
    ...
    if name == "start_job":
        ...
        exec_mode = args.get("mode", "mock")
        job_id = jobs.start_job(
            step, STEP_FUNCS[step], args, on_complete=_job_on_complete(run_id, step, phase, exec_mode, args)
        )
        state.record_step(run_id, step=step, status="started", phase=phase, exec_mode=exec_mode, inputs=args, job_id=job_id)
