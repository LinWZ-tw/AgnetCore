...
Reads results/<run_id>/state.json and writes three files to results/<run_id>/:
...
from agentcore import state as _state
...
def _wes_shell_block(step: str, sid: str, inp: dict, prov: dict, mode: str) -> str:
    ...
    if step == "qc":
        cmd = (
            f"{C} fastp \\\n"
            f"  -i {r1} -I {r2} \\\n"
            f"  -o results/qc/{sid}_R1.trimmed.fastq.gz \\\n"
            f"  -O results/qc/{sid}_R2.trimmed.fastq.gz \\\n"
            f"  --json results/qc/{sid}.fastp.json --thread 8"
        )
    elif step == "alignment":
        cmd = (
            f'{C} bash -c "\\\n'
            f"  bwa mem -t 8 -M {ref} \\\n"
            f"    results/qc/{sid}_R1.trimmed.fastq.gz \\\n"
            f"    results/qc/{sid}_R2.trimmed.fastq.gz \\\n"
            f'  | samtools sort -@ 4 -o results/alignment/{sid}.sorted.bam -"\n'
            f"{C} samtools index results/alignment/{sid}.sorted.bam"
        )
    elif step == "mutation_calling":
        cmd = (
            f"{C} gatk MarkDuplicates \\\n"
            f"  -I results/alignment/{sid}.sorted.bam \\\n"
            f"  -O results/mutation/{sid}.dedup.bam \\\n"
            f"  -M results/mutation/{sid}.dup_metrics.txt\n\n"
            f"{C} gatk BaseRecalibrator \\\n"
            f"  -I results/mutation/{sid}.dedup.bam \\\n"
            f"  -R {ref} \\\n"
            f"  --known-sites {dbsnp} \\\n"
            f"  -O results/mutation/{sid}.recal.table\n\n"
            f"{C} gatk ApplyBQSR \\\n"
            f"  -I results/mutation/{sid}.dedup.bam \\\n"
            f"  -R {ref} \\\n"
            f"  --bqsr-recal-file results/mutation/{sid}.recal.table \\\n"
            f"  -O results/mutation/{sid}.bqsr.bam\n\n"
            f"{C} gatk HaplotypeCaller \\\n"
            f"  -I results/mutation/{sid}.bqsr.bam \\\n"
            f"  -R {ref} \\\n"
            f"  -O results/mutation/{sid}.g.vcf.gz"
        )
    else:
...
def _scrna_shell_block(step: str, sid: str, inp: dict, prov: dict, mode: str) -> str:
    ...
    if step == "cell_annotation":
        ...
        code = (
            ...
            f"de_df.to_csv('results/diffexp/{sid}_de_results.csv')"
        )
    elif step == "gsea":
        code = (
            "import gseapy as gp, pandas as pd\n"
            f"de_df = pd.read_csv('results/diffexp/{sid}_de_results.csv', index_col=0)\n"
            ...
            ")\n"
            f"pre_res.res2d.to_csv('results/gsea/{sid}_gsea_results.csv')"
        )
    else:
...
def _build_snakefile(run_id: str, steps: list[dict]) -> str:
    ...
    targets = []
    if wes_samples:
        targets.append('        expand("results/mutation/{sample}.g.vcf.gz", sample=WES_SAMPLES),')
    if scrna_samples:
        targets.append('        expand("results/gsea/{sample}_gsea_results.csv", sample=SCRNA_SAMPLES),')
    ...
    if wes_samples:
        lines += [
            ...
            "        bam = 'results/alignment/{sample}.sorted.bam'",
            "    output:",
            "        vcf     = 'results/mutation/{sample}.g.vcf.gz',",
            "        dedup   = 'results/mutation/{sample}.dedup.bam',",
            "        metrics = 'results/mutation/{sample}.dup_metrics.txt'",
            ...
            "        ' -O results/mutation/{wildcards.sample}.recal.table && '",
            "        'gatk ApplyBQSR -I {output.dedup} -R {params.ref}'",
            "        ' --bqsr-recal-file results/mutation/{wildcards.sample}.recal.table'",
            "        ' -O results/mutation/{wildcards.sample}.bqsr.bam && '",
            "        'gatk HaplotypeCaller'",
            "        ' -I results/mutation/{wildcards.sample}.bqsr.bam'",
    ...
    if scrna_samples:
        lines += [
            ...
            "    output: h5ad = 'results/annotation/{sample}_annotated.h5ad'",
            f"    # scanpy v{TOOL_VERSIONS['scanpy']}",
            "    script: 'scripts/cell_annotation.py'",
            "",
            "rule clustering:",
            "    input:  h5ad = 'results/annotation/{sample}_annotated.h5ad'",
            "    output: h5ad = 'results/clustering/{sample}_clustered.h5ad'",
            ...
            "    input:  h5ad = 'results/clustering/{sample}_clustered.h5ad'",
            "    output: csv  = 'results/diffexp/{sample}_de_results.csv'",
            f"    # scanpy v{TOOL_VERSIONS['scanpy']}",
            "    script: 'scripts/differential_expression.py'",
            "",
            "rule gsea:",
            "    input:",
            "        de  = 'results/diffexp/{sample}_de_results.csv',",
            "        gmt = HALLMARK",
            "    output: csv = 'results/gsea/{sample}_gsea_results.csv'",
...
def _build_methods(run_id: str, steps: list[dict]) -> str:
    ...
    lines += [
        ...
        "",
        "All analyses were performed with the merged translational-agent framework's "
    ...
    lines += [
        "",
        f"Exact commands are provided in `results/{run_id}/reproduce.sh`. "
        f"A Snakemake workflow for re-running the analysis is in `results/{run_id}/Snakefile`.",
        "",
        f"*Run ID: `{run_id}`*",
    ]
...
def export_workflow(run_id: str) -> dict[str, str]:
    """Write reproduce.sh, Snakefile, and methods.md to results/<run_id>/."""
