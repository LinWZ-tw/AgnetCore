"""PRS Agent -- Stage 4: Clumping + Thresholding polygenic risk score."""
...
def run(
    *,
    ...
    provider = make_provider(
        ...
        system_prompt=SYSTEM_PROMPT, base_url=base_url, effort=effort, tools=PRS_TOOLS,
    )

    initial = (
        f"Run the PRS stage for run '{run_id}'.\n\n"
        f"Context from the Planner:\n{json.dumps(context, indent=2)}\n\n"
        f"Checkpoint so far:\n{state.summarize_state(run_id)}\n\n"
        "Begin by calling read_checkpoint to locate the formatted .ma file and "
        "the plink_bfile_ref from the V2G stage, then proceed."
    )
    provider.send_user_text(initial)
    state.append_log(run_id, {"event": "prs_start", "context": context})
    _emit(type="system", text="PRS stage started", agent=_LABEL)
    ...
    for _ in range(MAX_ITERATIONS):
        ...
        for call in result.tool_calls:
            ...
            try:
                output = dispatch(run_id, call["name"], call["input"], auto_approve, phase="prs")
    ...
    state.append_log(run_id, {"event": "prs_end", "summary": final_text[:500]})
    _emit(type="system", text="PRS stage complete", agent=_LABEL)
    return final_text
