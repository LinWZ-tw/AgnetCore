"""MR Agent -- Stage 2: SMR eQTL -> two-sample MR -> causal network."""
...
def run(
    *,
    ...
    provider = make_provider(
        ...
        system_prompt=SYSTEM_PROMPT, base_url=base_url, effort=effort, tools=MR_TOOLS,
    )

    initial = (
        f"Run the MR stage for run '{run_id}'.\n\n"
        f"Context from the Planner:\n{json.dumps(context, indent=2)}\n\n"
        f"Checkpoint so far:\n{state.summarize_state(run_id)}\n\n"
        "Begin by calling read_checkpoint to locate V2G outputs, then proceed."
    )
    provider.send_user_text(initial)
    state.append_log(run_id, {"event": "mr_start", "context": context})
    _emit(type="system", text="MR stage started", agent=_LABEL)
    ...
    for _ in range(MAX_ITERATIONS):
        ...
        for call in result.tool_calls:
            ...
            try:
                output = dispatch(run_id, call["name"], call["input"], auto_approve, phase="mr")
    ...
    state.append_log(run_id, {"event": "mr_end", "summary": final_text[:500]})
    _emit(type="system", text="MR stage complete", agent=_LABEL)
    return final_text
