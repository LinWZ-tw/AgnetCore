...
Two backends, selected automatically (override with ``AGENT0_EXEC_BACKEND``):
...
- ``wsl``    (default on Windows): commands are wrapped in ``wsl.exe`` and run
  ...
  *WSL* path (e.g. ``/home/<user>/agent0/...``), not a Windows path, because
...
Env-var overrides (all optional):
    AGENT0_EXEC_BACKEND   "wsl" | "native"
    AGENT0_WSL_DISTRO     distro name for ``wsl.exe -d`` (default: default distro)
    AGENT0_WSL_TOOLS_DIR  WSL path to the copied ``tools/`` dir
    AGENT0_CONDA_SH       WSL path to ``conda.sh`` sourced before each command
"""
...
_BACKEND = os.environ.get("AGENT0_EXEC_BACKEND") or ("wsl" if os.name == "nt" else "native")
_WSL_DISTRO = os.environ.get("AGENT0_WSL_DISTRO", "")
...
_CONDA_SH = os.environ.get("AGENT0_CONDA_SH", "$HOME/miniconda3/etc/profile.d/conda.sh")
_WSL_CD = os.environ.get("AGENT0_WSL_CD", "~")
...
if _BACKEND == "wsl":
    ...
    TOOLS_DIR: Any = PurePosixPath(
        os.environ.get("AGENT0_WSL_TOOLS_DIR", "/home/weizhilin/agent0/tools")
    )
