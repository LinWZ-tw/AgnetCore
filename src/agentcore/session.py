...
job / checkpoint stack `run_pipeline.py` uses, but as a long-lived,
interruptible chat instead of a single run-to-completion call.

A one-shot CLI run sends one goal and loops until the model stops calling
tools, then returns. That's fine for a CLI demo, but a GUI user needs to be
able to interject mid-run -- ask a question, correct the plan, hand over
more metadata -- without starting a new run. `AgentSession` keeps one
provider conversation alive in a background thread: each posted message
triggers the same tool-use loop, but the loop yields back to
"waiting_for_user" instead of exiting, so the conversation (and the
checkpoint it has been building under results/<run_id>/) just continues.
"""
