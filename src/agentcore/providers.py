...
`AgentSession` (session.py) and every domain's agents only talk to this
normalized interface: `send_user_text`, `send_tool_results`,
`step() -> TurnResult`. Each provider keeps the vendor-specific
message-history format internally and translates its own response shape
into a TurnResult so the rest of the loop (tool dispatch, checkpointing,
GUI events) never needs to know which vendor is behind it.
...
Supported out of the box:
  ...
  - "gemini" / "grok": route through OpenAIProvider with a fixed base_url.

This module has no dependency on any domain's tool schemas -- callers
(Planner, Worker/Stage agents, Reporter) always pass their own `tools=`
list explicitly.
"""
...
class AnthropicProvider:
    """Claude via the Messages API, with adaptive thinking and effort control."""
