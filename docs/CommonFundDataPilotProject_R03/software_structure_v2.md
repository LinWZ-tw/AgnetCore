# AgentGWAS: Software Architecture for the Agentic Post-GWAS Translational Pipeline

## Purpose

This document specifies the software architecture for **AgentGWAS**, an agentic post-GWAS analysis system designed to orchestrate a five-stage translational pipeline traversing variant-to-gene resolution, variant-to-transcriptome propagation, druggability assessment, in-silico perturbation simulation, and transcriptome-to-proteome projection. The system unifies twenty NIH Common Fund program datasets — spanning chromatin architecture (4DN), single-cell atlases (HuBMAP, SenNet), multi-tissue expression (GTEx, MoTrPAC), perturbation biology (LINCS), target pharmacology (IDG), longitudinal clinical cohorts (A2CPS, iHMP), phenotype-to-genotype resources (KOMP2), glycobiology (GlyGen), and pan-African genomics (H3Africa), among others — within a single LLM-orchestrated translational pipeline. The architecture is intended to support deployment on Amazon Web Services (AWS) and open-source distribution via GitHub, with the same codebase serving both targets through environment-configurable adapter interfaces. The system includes a graphical user interface through which a researcher may express an analysis intent in natural language; a planner agent that parses this input into a structured, human-reviewable session plan; and a task dispatch layer that assigns each approved plan step to a dedicated stage subagent.

---

## Design Principles

Eight principles govern the structural decisions described in this document. The first three correspond directly to the key innovations of AgentGWAS; the remaining five address operational and reproducibility requirements.

**Calibrated uncertainty propagation.** Each pipeline stage emits calibrated uncertainty scores that downstream stages must consume as continuous posteriors rather than discretized binary calls. The orchestration layer maintains a typed, persistent state object passed between stages that records evidence-weighted scores at every transition. A linear chain architecture is insufficient for this requirement; the orchestration framework must support conditional routing, back-propagation of corrections, and iterative refinement loops.

**Mandatory human-in-the-loop plan review.** Before any computational task is dispatched, the system presents the structured session plan to the user as a rendered markdown document. Execution proceeds only upon explicit approval. The user may edit the plan, reject it entirely, or request a revised plan from the planner agent. This checkpoint is not optional and is implemented as a LangGraph interrupt node rather than a courtesy prompt, ensuring it cannot be bypassed programmatically.

**Unified knowledge graph for multi-hop evidence queries.** All intermediate results are written to a property graph database as typed nodes and edges with uncertainty scores as edge properties. The knowledge graph serves as the shared memory of the pipeline across stages and across invocations, enabling multi-hop traversal queries that span the full variant-to-biomarker evidence chain from genetic variant to therapeutic target.
...
**Benchmark-driven validation.** The system must be evaluable against loci with established causal resolution. The benchmark suite targets type 2 diabetes loci (TCF7L2, SLC30A8) and lipid metabolic trait loci (PCSK9, APOC3), using recovery of known causal genes and identification of approved therapeutic targets as primary performance metrics. Fixture data for these loci are shipped in the test suite.
...
│                                                                 │
│   Stage 1 (V2G) ──► Stage 2 (V2T) ──┐                          │
│               └──► Stage 3 (Drug) ──┴──► Stage 4 (Pert) ──►    │
│                                              Stage 5 (Prot)     │
...
The pipeline topology is reflected in the following flowchart. Stages 2 (Variant-to-Transcriptome) and 3 (Druggability) execute concurrently once Stage 1 has emitted a gene list; both feed their outputs into Stage 4 before Stage 5 is dispatched.
...
│                                       #   Microbiome: NCBI Taxonomy, GTDB lineage
│
...
│   ├── sennet_client.py                # SenNet DCC API (senescence cell atlas)
│   ├── ihmp_client.py                  # iHMP / HMP2 portal REST client
│   │                                   #   Retrieves longitudinal microbiome and multi-omic
│   │                                   #   profiles; used in Stage 5 biomarker projection
...
│   └── fixtures/                       # Synthetic GWAS summary statistics for benchmark loci
│       ├── tcf7l2_t2d/                 # Type 2 diabetes: TCF7L2 locus (GRCh38)
│       ├── slc30a8_t2d/                # Type 2 diabetes: SLC30A8 locus (GRCh38)
│       ├── pcsk9_ldl/                  # LDL cholesterol: PCSK9 locus (GRCh38)
│       └── apoc3_tg/                   # Triglycerides: APOC3 locus (GRCh38)
│
├── docs/
...
## Common Fund Program Coverage

AgentGWAS integrates data from twenty NIH Common Fund programs. The table below maps each program to its primary pipeline stage(s) and the corresponding adapter or tool module.

| Common Fund Program | Category | Primary Stage(s) | Module |
|---|---|---|---|
| 4D Nucleome (4DN) | Chromatin architecture | 1 | `data_access/four_dn_client.py` |
| HuBMAP | Single-cell atlas | 1, 2 | `data_access/hubmap_client.py` |
| SenNet | Single-cell atlas (senescence) | 1, 2 | `data_access/sennet_client.py` |
| GTEx | Multi-tissue eQTL | 2 | `tools/gtex_api.py` |
| MoTrPAC | Exercise multi-omics | 2, 5 | `tools/motrpac_loader.py` |
| LINCS | Perturbation biology | 4 | `tools/lincs_api.py`, `tools/connectivity_score.py` |
| IDG | Target pharmacology | 3 | `tools/pharos_api.py` |
| A2CPS | Longitudinal pain cohort | 5 | `data_access/anvil_client.py` |
| iHMP | Longitudinal microbiome cohort | 5 | `data_access/ihmp_client.py` |
| KOMP2 / IMPC | Mouse phenotyping | 3 | `tools/komp2_api.py` |
| GlyGen | Glycobiology | 3, 5 | `tools/glygen_api.py` |
| H3Africa | Pan-African genomics | 1 | `data_access/ega_client.py` |
| Kids First | Pediatric genomics | 1 | `data_access/anvil_client.py` |
| UDN | Undiagnosed diseases | 1 | `data_access/anvil_client.py` |
| CFDE | Cross-program metadata | All | `data_access/cfde_portal.py` |
...
Stages that have no inter-stage dependency on the output of an earlier stage are dispatched concurrently. Specifically, Stage 2 (Variant-to-Transcriptome) and Stage 3 (Druggability) both depend only on the gene list emitted by Stage 1 and are therefore submitted as parallel tasks as soon as Stage 1 exits. Stage 4 (In-Silico Perturbation) is dispatched only after both Stage 2 and Stage 3 have written their outputs to the shared `PipelineState`. Stage 5 (Transcriptome-to-Proteome) is dispatched after Stage 4 completes. The task dispatcher inspects the dependency graph encoded in the `SessionPlan` to enforce this ordering.
...
The shared state object, defined in `state/pipeline_state.py`, is a Pydantic model whose fields correspond to the principal evidence structures produced and consumed across stages. Every node in every subgraph receives the current state as input and returns a partial state update as output; LangGraph merges these updates according to the declared reducer functions. Uncertainty flags accumulated during execution trigger conditional edges that route the agent back to an earlier stage for supplementary data retrieval rather than proceeding forward with insufficient evidence. These uncertainty scores are propagated forward as continuous posteriors, realizing the calibrated uncertainty propagation design principle.
...
The knowledge graph is the shared persistent memory of the pipeline and the primary substrate for multi-hop evidence queries from genetic variant to therapeutic target. The schema, defined in `knowledge_graph/schema.py`, specifies eleven node types and seven edge types.

**Node types:** Variant, CredibleSet, Gene, Tissue, CellType, Drug, Compound, Protein, Phenotype, Pathway, Microbiome.

**Edge types and uncertainty properties:**

| Edge type | Source → Target | Uncertainty property |
|---|---|---|
| MAPS_TO | Variant → Gene | multi-modal posterior score |
| COLOCALIZES_WITH | Variant → Gene (via eQTL) | H4 posterior probability |
| REGULATES | Gene → Gene / Pathway | TWAS Z-score, CV R² |
| TARGETS | Drug → Gene / Protein | Pharos target development level |
| PERTURBS | Compound → Gene | LINCS connectivity tau, p-value |
| PREDICTS_BIOMARKER | Protein → Phenotype | hazard ratio, confidence interval, cohort |
| ASSOCIATES_WITH | Microbiome → Phenotype | correlation coefficient, study ID |
...
**Access platform identities.** The four controlled-access platforms referenced throughout this document are authorization infrastructure, not NIH Common Fund programs. *dbGaP* (Database of Genotypes and Phenotypes) is an NCBI-managed repository for controlled-access human genetic and phenotypic data; many Common Fund studies deposit their protected-access tiers here (including GTEx protected-access and H3Africa), but dbGaP itself is NIH-wide infrastructure. *EGA* (European Genome-phenome Archive) is operated by EMBL-EBI and the Centre for Genomic Regulation; it is a European infrastructure entirely outside the NIH Common Fund, used here solely because H3Africa deposits data there. *AnVIL* is a cloud genomic analysis platform funded specifically by NHGRI, not the Common Fund; it hosts GTEx, Kids First, UDN, and A2CPS datasets under a Terra workspace model. *BioData Catalyst* is an NHLBI-funded cloud ecosystem that hosts MoTrPAC and other cardiovascular and pulmonary datasets. All four function as access gateways through which AgentGWAS retrieves Common Fund data; none contributes data independently.

The `ihmp_client.py` adapter retrieves longitudinal multi-omic profiles from the Human Microbiome Project Phase 2 (iHMP) portal. These include metagenomic, metatranscriptomic, metabolomic, and proteomics time-series data for the IBDMDB, T2D, and MOMS-PI cohorts. The adapter is consumed by the Stage 5 subagent when projecting transcriptomic signals onto microbiome-associated biomarker candidates.
...
The primary validation strategy targets four loci with established causal resolution and approved therapeutic targets. Benchmark fixtures are shipped in `tests/fixtures/` as synthetic GWAS summary statistics constructed from published association data at GRCh38 coordinates.

| Trait | Locus | Expected causal gene | Approved target |
|---|---|---|---|
| Type 2 diabetes | TCF7L2 | TCF7L2 | — (transcription factor) |
| Type 2 diabetes | SLC30A8 | SLC30A8 | — (loss-of-function protective) |
| LDL cholesterol | PCSK9 | PCSK9 | Evolocumab, Alirocumab |
| Triglycerides | APOC3 | APOC3 | Olezarsen |

Performance metrics evaluated are (1) recovery rate of the expected causal gene within the top-ranked Stage 1 output, (2) identification of the approved drug as the top-ranked Stage 3 druggability hit, and (3) full end-to-end pipeline completion without error across all four loci.
...
**Milestone 1: Core infrastructure.** Implement the knowledge graph schema (including the Microbiome node type and ASSOCIATES_WITH edge), Neo4j client, and identifier harmonizer. Define the `PipelineState` and `SessionPlan` Pydantic models. Set up the `docker-compose.yml` stack. This establishes the persistence and configuration foundation on which all other components depend.

**Milestone 2: Planner agent and GUI scaffold.** Implement the planner agent with structured output mode against a small set of example queries, including the four benchmark loci. Implement `plan_writer.py` and verify that a `session_plan.md` is generated correctly. Build the Streamlit pages for query input and plan review. Test the human-in-the-loop interrupt node in LangGraph in isolation. This milestone delivers the user-facing entry point and validates the NLU parsing before any pipeline compute is involved.
...
**Milestone 4: Stage 1.** Implement the fine-mapping Nextflow workflow, the 4DN and HuBMAP API clients, and the Stage 1 subgraph. Stage 1 produces the credible set input that Stage 2 consumes, closing the upstream dependency and enabling end-to-end runs from raw GWAS summary statistics. Validate against all four benchmark loci.

**Milestone 5: Stages 3, 4, and 5.** Implement the remaining three stages in order. Stage 3 (Druggability) has no Nextflow dependency and can be completed quickly using the Pharos GraphQL client; validate recovery of evolocumab against PCSK9. Stage 4 requires the LINCS GCTx loader and the connectivity scoring workflow. Stage 5 draws on the MoTrPAC, A2CPS, and iHMP data adapters. Verify the parallel dispatch of Stages 2 and 3 by asserting that their task start timestamps overlap in the audit log.
