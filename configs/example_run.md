...
(`~/agentCore/data/...`) that the GWAS steps require under the Windows->WSL execution backend
(see README "GWAS conda environments") - substitute your own trait/data.

```
python run_pipeline.py --input /home/<user>/agentCore/data/gwas/<trait>.ma --trait MASLD \
  --run-id MASLD_run --goal "
Run the post-GWAS pipeline for MASLD:

1. The exposure GWAS is already in GCTA .ma format at
   /home/<user>/agentCore/data/gwas/<trait>.ma (sample size 778614) - skip format_gwas_to_ma.
   (If starting from raw summary stats instead, run format_gwas first.)
2. Run COJO against the 1000G hg38 reference panel at
   /home/<user>/agentCore/data/ref/1000G_phase3/all_hg38. This is the full ALL-VARIANTS panel
...
4. Validate SNP -> gene links via SMR against GTEx liver eQTLs at
   /home/<user>/agentCore/data/ref/gtex_v8/Liver.v8.signif_variant_gene_pairs.txt.gz
...
Put all outputs under /home/<user>/agentCore/results/MASLD_run/. Tell me what you find, including
...
```json
{
  "locus_master_file": "/home/<user>/agentCore/results/MASLD_run/COJO/COJO_MASLD_locus_master.csv",
  "sumstats_map": {
    "MASLD": {"file": "/home/<user>/agentCore/data/gwas/<trait>.ma", "N": 778614}
  },
  "window_size": 500000,
  "out_base_dir": "/home/<user>/agentCore/results/MASLD_run/SuSiE",
  "bfile": "/home/<user>/agentCore/data/ref/1000G_phase3/all_hg38",
  "conda_env": "finemap",
  "cojo_locus_map_template": "/home/<user>/agentCore/results/MASLD_run/COJO/COJO_{trait}_Locus_Map.xlsx"
}
```
