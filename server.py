#!/usr/bin/env python3
"""Local web server bridging the GUI (static/index.html) to the merged
agent's chat-session / tool / job / checkpoint stack.

Serves both domains (bio WES/scRNA and post-GWAS) through one server, one
port, and one generic "Input path" form field -- the Planner auto-detects
which domain applies (see agentcore/agents/planner.py).
...
from agentcore import RESULT_DIR, state  # noqa: E402
from agentcore.session import AgentSession  # noqa: E402
...
DEFAULT_GOAL = (
    ...
    "QC/alignment/mutation-calling for WES, cell-annotation/clustering/"
    "differential-expression/GSEA for scRNA, or format_gwas/COJO/SuSiE -> "
    "SMR eQTL/two-sample MR -> druggability for GWAS summary stats."
)


def _build_initial_message(run_id: str, *, input_path: str, goal: str, notes: str) -> str:
    parts = [f"Goal: {goal}", f"Primary input path to start from: {input_path}"]
    if notes.strip():
        parts.append(f"Additional context provided by the user (study design, trait, sample notes):\n{notes.strip()}")
    ...
    parts.append(
        ...
        "then proceed through the appropriate pipeline."
    )
    return "\n\n".join(parts)


class Handler(BaseHTTPRequestHandler):
    server_version = "AgentCoreGUI/1.0"
    ...
    def do_GET(self) -> None:  # noqa: N802 - stdlib method name
        ...
        if m:
            ...
            return self._send_json({
                "ready": ready,
                "url": f"/results/{run_id}/report/report.html" if ready else None,
            })

        # --- serve report files (HTML, Markdown, PNG figures, network diagrams) ---
        m = re.match(r"^/results/([^/]+)/report/(.+)$", path)
        if m:
    ...
    def do_POST(self) -> None:  # noqa: N802 - stdlib method name
        ...
        if path == "/api/sessions":
            ...
            input_path = body.get("input_path", "") or body.get("data_path", "") or body.get("gwas_file", "")
            checkpoint_exists = (RESULT_DIR / run_id / "state.json").exists()
            if checkpoint_exists and not input_path.strip():
                initial = (
                ...
                notes = body.get("notes", "") or body.get("study_design", "") or body.get("sample_notes", "") or body.get("trait", "")
            ...
            return self._send_json({
                "run_id": run_id,
                "status": session.status,
                "resumed": checkpoint_exists and not input_path.strip(),
            })
